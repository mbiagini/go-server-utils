...
// ClientKey is the type used to set and retrieve a Client from the internal clients
// map.
...
func GetClient(key ClientKey) (c *Client, ok bool) {
	c, ok = clients[key]
	return c, ok
}
...
// ValidateRequiredClients receives a list of client keys and checks if all of
// them correspond to clients already configured in the internal clients map. If
// any of them is missing, returns an error.
//
// Should be called from a configuration file after loading the clients.
func ValidateRequiredClients(requiredKeys []ClientKey) error {
	for _, requiredKey := range requiredKeys {
		if _, ok := GetClient(requiredKey); !ok {
			return fmt.Errorf("could not find client with key %s configured", string(requiredKey))
		}
	}
	return nil
}
